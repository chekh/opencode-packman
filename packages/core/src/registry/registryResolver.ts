...
function parseRegistryReference(reference: string): { registryName: string; packageName: string } | null {
...
export async function resolvePackageReference(input: ResolveInput): Promise<ResolveOutput> {
  ...
  if (registry === undefined) {
    throw new Error(`Unknown registry '${parsed.registryName}'. Run 'opm registry list' to inspect configured registries.`);
  }

  if (registry.type !== 'local') {
    throw new Error(`Unsupported registry type '${String(registry.type)}' for '${parsed.registryName}'.`);
  }

  const packageRoot = path.resolve(registry.path, 'packages', parsed.packageName);
