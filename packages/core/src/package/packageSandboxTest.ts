...
import { validatePackage, type ValidationMessage, type ValidationResult } from './packageValidator.js';
...
function appendStep(steps: PackageSandboxTestStep[], stage: string, status: PackageSandboxTestStepStatus, message?: string): void {
  steps.push(message === undefined ? { stage, status } : { stage, status, message });
}
...
export async function runPackageSandboxTest(input: {
  ...
  function buildResult(status: PackageSandboxTestStepStatus): PackageSandboxTestResult {
    return {
  ...
  try {
    const resolved =
      input.baseDir === undefined
        ...
        : await resolvePackageReference({ reference: input.packageRef, baseDir: input.baseDir });
    ...
    const installPlan = await buildInstallPlan({ packageRoot, projectRoot: sandboxRoot });
    if (!installPlan.validation.ok || installPlan.conflicts.length > 0) {
      const detail = installPlan.conflicts.length > 0 ? 'Install plan has conflicts.' : 'Install plan validation failed.';
      errors.push(detail);
    ...
    if (!installResult.ok) {
      const detail = installResult.errors.map((error) => error.message).join('; ');
      errors.push(detail);
    ...
    warnings.push(...doctorBeforeRemove.issues.filter((issue) => issue.severity === 'warning'));
    ...
    appendStep(steps, 'doctor before remove', doctorBeforeRemove.status === 'warning' ? 'warning' : 'ok');

    const removePlan = await buildRemovePlan({ projectRoot: sandboxRoot, packageName: loaded.manifest.name });
    ...
    if (!removeResult.ok) {
      const detail = removeResult.errors.map((error) => error.message).join('; ');
      errors.push(detail);
    ...
    warnings.push(...doctorAfterRemove.issues.filter((issue) => issue.severity === 'warning'));
    ...
    appendStep(steps, 'doctor after remove', doctorAfterRemove.status === 'warning' ? 'warning' : 'ok');

    const status: PackageSandboxTestStepStatus =
      warnings.length > 0 || steps.some((step) => step.status === 'warning') ? 'warning' : 'ok';
