...
describe('doctor', () => {
  it('reports warning when opencode.json is missing', async () => {
    ...
    expect(
      report.issues.some((issue) => issue.code === 'missing_opencode_json'),
    ).toBe(true);
  ...
  it('reports warning when lockfile is missing', async () => {
    ...
    expect(
      report.issues.some((issue) => issue.code === 'missing_lockfile'),
    ).toBe(true);
    expect(report.status).toBe('warning');
  });
  ...
  it('reports error when locked file is deleted', async () => {
    ...
    expect(
      report.issues.some((issue) => issue.code === 'missing_locked_target'),
    ).toBe(true);
  });
  ...
  it('reports error when opencode.json is invalid JSON', async () => {
    ...
    expect(report.status).toBe('broken');
    expect(
      report.issues.some((issue) => issue.code === 'invalid_opencode_json'),
    ).toBe(true);
  });
  ...
  it('reports error when skill directory lacks SKILL.md', async () => {
    ...
    expect(report.status).toBe('broken');
    expect(
      report.issues.some((issue) => issue.code === 'missing_skill_file'),
    ).toBe(true);
  });
  ...
  it('reports warning when package has no owned targets', async () => {
    ...
    expect(
      report.issues.some(
        (issue) => issue.code === 'package_has_no_owned_targets',
      ),
    ).toBe(true);
  });
  ...
  it('reports warning when baseline is missing in initialized project', async () => {
    ...
    expect(report.status).toBe('warning');
    expect(
      report.issues.some((issue) => issue.code === 'missing_baseline'),
    ).toBe(true);
  });
  ...
  it('does not treat baseline files as installed package targets', async () => {
    ...
    await fs.writeFile(
      path.join(projectRoot, '.opencode/agents/existing.md'),
      'existing\n',
      'utf8',
    );
    await initProject(projectRoot);
  ...
  it('reports baseline_file_modified when baseline checksum changes', async () => {
    ...
    expect(report.status).toBe('warning');
    expect(
      report.issues.some((issue) => issue.code === 'baseline_file_modified'),
    ).toBe(true);
  });
  ...
  it('reports baseline_file_missing when baseline file is deleted', async () => {
    ...
    expect(report.status).toBe('warning');
    expect(
      report.issues.some((issue) => issue.code === 'baseline_file_missing'),
    ).toBe(true);
  });
  ...
  it('reports warning when installed file is modified after install', async () => {
    ...
    expect(report.status).toBe('warning');
    expect(
      report.issues.some((issue) => issue.code === 'locked_target_modified'),
    ).toBe(true);
  });
