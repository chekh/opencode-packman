...
describe('create package scaffold', () => {
  it('create bundle scaffold creates expected files', async () => {
    ...
    expect(await fs.pathExists(path.join(targetDir, 'package.yaml'))).toBe(
      true,
    );
    expect(
      await fs.pathExists(
        path.join(targetDir, 'agents/backend-review-reviewer.md'),
      ),
    ).toBe(true);
    expect(
      await fs.pathExists(
        path.join(targetDir, 'commands/backend-review-review.md'),
      ),
    ).toBe(true);
    expect(
      await fs.pathExists(
        path.join(targetDir, 'skills/backend-review-skill/SKILL.md'),
      ),
    ).toBe(true);
    expect(
      await fs.pathExists(path.join(targetDir, 'opencode.patch.json')),
    ).toBe(true);
  });
  ...
  it('create skill scaffold creates SKILL.md with valid frontmatter', async () => {
    ...
    const skillRaw = await fs.readFile(
      path.join(targetDir, 'skills/api-review/SKILL.md'),
      'utf8',
    );
    expect(skillRaw).toContain('name: api-review');
    expect(skillRaw).toContain(
      'description: "TODO: describe the api-review skill"',
    );
  });
  ...
  it('create agent scaffold creates agent file and valid package.yaml', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(
      await fs.pathExists(path.join(targetDir, 'agents/security-auditor.md')),
    ).toBe(true);
  ...
  it('create command scaffold creates command file and valid package.yaml', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(
      await fs.pathExists(path.join(targetDir, 'commands/review.md')),
    ).toBe(true);
  ...
  it('create profile scaffold creates opencode.patch.json and valid package.yaml', async () => {
    ...
    expect(loaded.manifest.exports.config?.[0]?.path).toBe(
      'opencode.patch.json',
    );
  });
  ...
  it('invalid package name is rejected', async () => {
    ...
    expect(
      result.errors.some((error) => error.code === 'invalid_package_name'),
    ).toBe(true);
    expect(await fs.pathExists(path.join(targetDir, 'package.yaml'))).toBe(
      false,
    );
  ...
  it('target non-empty dir fails without force', async () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.errors.some((error) => error.code === 'target_exists')).toBe(
      true,
    );
    expect(await fs.pathExists(path.join(targetDir, 'package.yaml'))).toBe(
      false,
  ...
  it('target path as file is rejected', async () => {
    ...
    expect(result.ok).toBe(false);
    expect(
      result.errors.some((error) => error.code === 'target_not_directory'),
    ).toBe(true);
  });
  ...
  it('target non-empty dir works with force', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(await fs.pathExists(path.join(targetDir, 'existing.txt'))).toBe(
      true,
    );
    expect(await fs.pathExists(path.join(targetDir, 'package.yaml'))).toBe(
      true,
  ...
  it('force mode does not overwrite existing scaffold file', async () => {
    ...
    await fs.ensureDir(targetDir);
    await fs.writeFile(
      path.join(targetDir, 'package.yaml'),
      'custom: true\n',
      'utf8',
    );
    ...
    expect(
      result.warnings.some(
        (warning) => warning.code === 'target_file_exists_skipped',
      ),
    ).toBe(true);
    expect(
      await fs.readFile(path.join(targetDir, 'package.yaml'), 'utf8'),
    ).toBe('custom: true\n');
  });
  ...
  it('scaffold rejects existing symlink directory that escapes target root', async () => {
    ...
    expect(result.ok).toBe(false);
    expect(
      result.errors.some((error) => error.code === 'unsafe_target_path'),
    ).toBe(true);
    expect(await fs.pathExists(path.join(outsideDir, 'pkg-reviewer.md'))).toBe(
      false,
    );
  });
  ...
  it('scaffold target cannot escape target dir', async () => {
    ...
    for (const createdPath of createdPaths) {
      ...
      expect(
        resolved === targetDir ||
          resolved.startsWith(`${targetDir}${path.sep}`),
      ).toBe(true);
    }
  });
  ...
  it('registry create resolves target to registry.path/packages/<name>', async () => {
    ...
    expect(resolved.targetDir).toBe(
      path.resolve(registryRoot, 'packages/demo-review'),
    );
  });
});
