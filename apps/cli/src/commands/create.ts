...
const ALLOWED_TYPES: CreatePackageType[] = [
  'skill',
  'agent',
  'command',
  'bundle',
...
function parseCreateType(value: string): CreatePackageType {
  ...
  throw new InvalidArgumentError(
    `Invalid package type '${value}'. Allowed: ${ALLOWED_TYPES.join(', ')}`,
  );
}
...
Examples:
  ...
  create
    ...
    .option(
      '--registry <name>',
      'Create at <registry.path>/packages/<name> from registry config',
    )
    .option(
      '--force',
      'Allow non-empty target directory (no deletion of existing files)',
      false,
    )
    .addHelpText(
      'after',
  ...
  opm create package backend-review --registry personal --force`,
    ...
    .action(async (name: string, options: CreatePackageOptions) => {
      try {
        ...
        const createdFiles = createResult.filesCreated.map((filePath) =>
          path.relative(createResult.packageRoot, filePath),
        );
        const lines = [
        ...
        if (resolvedTarget.registryName !== undefined) {
          lines.push(
            `  opm preview ${resolvedTarget.registryName}/${createResult.packageName}`,
          );
          lines.push(
            `  opm install ${resolvedTarget.registryName}/${createResult.packageName} --yes`,
          );
        } else {
